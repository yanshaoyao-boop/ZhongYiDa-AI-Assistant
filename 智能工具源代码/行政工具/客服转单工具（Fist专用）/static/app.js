...
const imageFileCard = document.getElementById("imageFileCard");
const imageTargetSelect = document.getElementById("imageTargetSelect");
...
// 点击卡片触发上传
[
  { card: aFileCard, input: aFileInput },
  { card: bFileCard, input: bFileInput },
  { card: imageFileCard, input: imageFileInput }
].forEach(({ card, input }) => {
...
imageFileInput.addEventListener("change", () => {
  ...
  imageFileName.textContent = file ? file.name : "未选择图片";
  imageFileCard.classList.toggle("is-ready", !!file);
});
...
async function inspectFiles() {
  if (!aFileInput.files[0] || !bFileInput.files[0]) {
    setGlobalStatus("请先选择 A 表和 B 表。", true);
    setFileStatus("还缺少文件，请确认 A 表和 B 表都已选择。", true);
    return;
  }

  setGlobalStatus("正在读取并解析 Excel 字段...", false);
  setFileStatus("文件已选齐，正在读取字段。", false);
  setMappingStatus("正在读取并解析 Excel 字段...", false);
  inspectButton.disabled = true;
  inspectButton.textContent = "读取中...";
  ...
  try {
    ...
    const response = await fetch(getApiUrl("/api/inspect"), {
      method: "POST",
    ...
    const msg = `字段读取完成：A 表 ${state.headers.length} 个字段，B 表 ${state.targets.length} 个目标位。`;
    setGlobalStatus(msg, false);
    setMappingStatus(msg, false);
    updateActionState();
  } catch (error) {
    console.error(error);
    ...
    inspectButton.textContent = "读取字段";
  }
}
...
function addMappingRow(sourceHeader = "", targetCell = "") {
  if (state.mappingRows.length >= 30) {
    setGlobalStatus("最多只能配置 30 条映射。", true);
...
function refreshImageTargetOptions() {
  const currentTarget = imageTargetSelect.value;
  imageTargetSelect.innerHTML = '<option value="">选择 B 目标格子</option>';
  state.targets.forEach((target) => {
    const option = document.createElement("option");
    option.value = target.cell;
    option.textContent = `${target.label} [${target.cell}]`;
    option.selected = target.cell === currentTarget;
    imageTargetSelect.appendChild(option);
  });
}
...
function exportConfig() {
  ...
  if (!mappings.length) {
    setGlobalStatus("当前没有可导出的映射配置。", true);
    return;
  }
...
async function importConfig(event) {
  ...
  try {
    ...
    setGlobalStatus("映射配置已导入。读取完字段后会自动匹配下拉选项。", false);
    setMappingStatus("映射配置已导入。读取完字段后会自动匹配下拉选项。", false);
  } catch (error) {
    setGlobalStatus(`导入配置失败: ${error.message}`, true);
...
async function generateFiles() {
  ...
  const mappings = collectMappings();
  setGlobalStatus("正在生成文件，请稍候...", false);
  ...
  try {
    ...
    if (imageFileInput.files[0] && imageTargetSelect.value) {
      payload.imageConfig = {
        imageName: imageFileInput.files[0].name,
        imageContent: await fileToBase64(imageFileInput.files[0]),
        targetCell: imageTargetSelect.value,
      };
    }
...
function setGlobalStatus(message, isError) {
  const html = `<p>${isError ? '⚠️ ' : ''}${escapeHtml(message)}</p>`;
...
function updateFileSelectionState() {
  ...
  const hasImage = Boolean(imageFileInput.files[0]);

  aFileCard.classList.toggle("is-ready", hasA);
  bFileCard.classList.toggle("is-ready", hasB);
  imageFileCard.classList.toggle("is-ready", hasImage);
