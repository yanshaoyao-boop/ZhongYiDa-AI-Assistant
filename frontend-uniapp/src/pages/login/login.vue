      ...
      <view class="zen-login-card login-container">
        <view class="brand-header">
          ...
          <text v-if="showLoopbackWarning" class="api-base-warning">
            当前地址仅开发者工具可用，真机请长按上方 Logo 改为局域网地址，例如 http://192.168.x.x:8000
          </text>
        </view>
        ...
        <view v-if="showDebugAuthHint" class="debug-auth-tip">
          <text>当前预览环境检测到本地登录态，已临时关闭自动跳转，便于排查白屏。</text>
        </view>

        <button v-if="showDebugContinueButton" class="debug-continue-btn" @tap="goToChatManually">
          <text>继续进入聊天页</text>
        </button>
...
const miniProgramEnvVersion = ref('unknown')

const isPreviewLikeEnv = computed(() => {
  return miniProgramEnvVersion.value === 'develop' || miniProgramEnvVersion.value === 'trial'
})

const shouldSkipAutoRedirect = computed(() => {
  return isPreviewLikeEnv.value
})

const showDebugAuthHint = computed(() => {
  return shouldSkipAutoRedirect.value && auth.isAuthenticated
})

const showDebugContinueButton = computed(() => {
  return shouldSkipAutoRedirect.value && auth.isAuthenticated
})
  ...
  uni.showModal({
    ...
    success: (res) => {
      ...
      if (!/^https?:\/\//i.test(nextValue)) {
        uni.showToast({ title: '地址需要以 http:// 或 https:// 开头', icon: 'none' })
        return
      }
...
const goToChatManually = () => {
  uni.reLaunch({ url: '/pages/chat/chat' })
}

const detectMiniProgramEnvVersion = () => {
  // #ifdef MP-WEIXIN
  try {
    const accountInfo = wx.getAccountInfoSync?.()
    miniProgramEnvVersion.value = accountInfo?.miniProgram?.envVersion || 'unknown'
  } catch (error) {
    miniProgramEnvVersion.value = 'unknown'
  }
  // #endif
}
...
onMounted(() => {
  ...
  syncApiBaseState()
  detectMiniProgramEnvVersion()
  if (auth.isAuthenticated && !shouldSkipAutoRedirect.value) {
    uni.reLaunch({ url: '/pages/chat/chat' })
  }
})
</script>
