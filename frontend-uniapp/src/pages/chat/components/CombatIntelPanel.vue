<template>
  <view class="intel-root">
    <view v-if="isOpen && isCollapsed" class="combat-intel-minimized glass-panel" @tap="$emit('setcollapsed', false)">
      <text class="minimized-icon">📌</text>
      <text class="minimized-text">实战情报</text>
    </view>

    <view v-if="isOpen" :class="['combat-intel-panel', 'combat-intel-shell', 'glass-panel', { collapsed: isCollapsed }]">
      <view class="panel-header">
        <text class="panel-title">实战情报中心</text>
        <view class="panel-header-btns">
          <view class="panel-collapse-btn" @tap="$emit('setcollapsed', true)">−</view>
          <view class="panel-close" @tap="$emit('setopen', false)">×</view>
        </view>
      </view>

      <scroll-view v-if="!isCollapsed" scroll-y class="panel-content">
        ...
        <view class="intel-section">
          ...
          <text v-else class="intel-text empty-hint">先进入一个教练场景，情报会自动显示在这里。</text>
        </view>

        <button v-if="currentScenario" class="quit-combat-btn" @tap="$emit('quit')">结束对练并点评</button>
      </scroll-view>
    </view>
  </view>
</template>
...
defineEmits(['setcollapsed', 'setopen', 'quit'])
</script>
